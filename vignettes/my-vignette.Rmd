...
This package depends on three other packages:
...
The package provides an RC class named **cityweather** to use weather data.
In order to create an object of class, you need to give a character name of the city you want to analyze. 

```{r, eval=FALSE}
weatherforecast::cityweather$new('Baghdad')
```
...
```{r, eval=FALSE}
httr::status_code(Respsone_Data) == 404
```

For checking the response code. It Extract status code from response. **200** response code is for proper data whereas **404** is for error.

```{r, eval=FALSE}
httr::http_type(Respsone_Data) != 'application/json'
```

For checking the type of data. It Extract Content type of the response. Here we are checking if our data is of JSON format or not.

```{r, eval=FALSE}
httr::http_error(Respsone_Data)
```

This part checks for an http error. It returns TRUE if the request fails (status code 400 or above), otherwise FALSE.
